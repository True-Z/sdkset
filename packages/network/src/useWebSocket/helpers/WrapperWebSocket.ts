...
import type { CreateWebSocketDefaults, WebSocketInterceptor } from '../types'
...
export class WrapperWebSocket {
  readonly #config: Required<CreateWebSocketDefaults>
  ...
  constructor(config: Required<CreateWebSocketDefaults>, observer: WrapperObserver) {
  ...
  #initInterceptors(interceptors: WebSocketInterceptor = {}) {
  ...
  mountInterceptors(interceptors: WebSocketInterceptor = {}) {
  ...
  /** 卸载`interceptors`，发布特定事件，处理边界情况。 */
  unmountInterceptors() {
    this.#observer.cancelWhole()
  }
