...
/**
 * 返回给定集合中的最小值。如果传递`iteratee`参数，`iteratee`将作为给定集合中值的排序依据。
 ...
 */
export function min<V extends Collection, I extends Iteratee<V, unknown>>(list: V, iteratee?: I, context?: unknown) {
  if (list == null) {
    return Infinity
  }
  let result = Infinity
  ...
  return result as TypeOfCollection<V> | number
}
